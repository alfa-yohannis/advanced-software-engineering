...
\subsection*{Sesi 5 --- Kode sebagai Infrastruktur (Infrastructure as Code)}

\textbf{Deskripsi:} Infrastruktur sistem tidak lagi diperlakukan sebagai konfigurasi manual atau sekadar file orkestrasi lokal, melainkan sebagai kode deklaratif yang mendefinisikan kondisi akhir (\textit{desired state}) secara eksplisit. Sesi ini membahas bagaimana pendekatan Infrastructure as Code (IaC) menggunakan Terraform memungkinkan konsistensi, reproducibility, dan skalabilitas pada TTS microservice stack. Penekanan diberikan pada pengelolaan multi-environment (\texttt{dev}, \texttt{staging}, \texttt{production}), pemisahan konfigurasi dari implementasi, serta pengendalian perubahan infrastruktur secara terstruktur.
\textbf{Alat:} Terraform sebagai alat IaC deklaratif, konsep module dan variable untuk reuse konfigurasi lintas environment, serta alur kerja \textit{plan} dan \textit{apply} untuk mengontrol perubahan infrastruktur.
\textbf{Mahasiswa Non-IT:} Mahasiswa memahami bahwa komponen-komponen dari suatu sistem perangkat lunak dapat dipandang sebagai bagian dari model infrastruktur yang terdokumentasi dalam kode. Mahasiswa mampu menjelaskan mengapa pemisahan environment dan konfigurasi berbasis variabel penting untuk menjaga konsistensi dan menghindari konflik deployment.
\textbf{Mahasiswa IT:} Mahasiswa menganalisis bagaimana struktur modular dan konfigurasi berbasis variabel mendukung deployment multi-environment secara konsisten. Mahasiswa memahami prinsip environment parity, kontrol perubahan melalui mekanisme perencanaan (\textit{plan}), serta konsep scaling horizontal melalui parameter konfigurasi tanpa mengubah arsitektur dasar sistem.
