...
locals {
  ...
  overlay_path = "${var.project_root}/iac/kubernetes/overlays/${var.k8s_overlay}"
  ...
  k8s_manifest_hash = sha1(join("", [
    for f in sort(fileset("${var.project_root}/iac/kubernetes", "**/*.yaml")) :
    filesha1("${var.project_root}/iac/kubernetes/${f}")
  ]))
...
resource "terraform_data" "deploy" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      kubectl -n tts scale deployment/worker --replicas=${var.worker_replicas}
    EOT
  }
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      set -euo pipefail
      kubectl delete -k "${self.input.project_root}/iac/kubernetes/overlays/${self.input.k8s_overlay}" --ignore-not-found=true
    EOT
  }
}
