...
\begin{tikzpicture}[>=Latex, node distance=12mm,
  ...
  \draw[->, dashed] (mm1) -- (m1) node[midway,right,font=\tiny]{conforms to};
  \draw[->, dashed] (mm2) -- (m2) node[midway,left,font=\tiny]{conforms to};
  \draw[->, dashed] (mm1.east) -- (trans.north west) node[midway,above,font=\tiny,sloped]{reads};
  \draw[->, dashed] (mm2.west) -- (trans.north east) node[midway,above,font=\tiny,sloped]{produces};
\end{tikzpicture}
}
...
\begin{frame}{Transformasi: MediaFlow $\rightarrow$ UML Activity Diagram}
\vspace{6pt}
\begin{columns}[T]
\column{0.5\textwidth}
...
\begin{frame}{Visualisasi Pemetaan \texttt{flow2}}
\vspace{2pt}
\begin{figure}
\centering
\scalebox{0.78}{
...
\caption{Visualisasi pemetaan model \texttt{flow2} dari MediaFlow ke UML Activity Diagram.}
...
<mediaflow:Graph name="demo">
  ...
  <nodes xsi:type="mediaflow:Scaler"
      name="resize720"
      command="ffmpeg -i input.mp4 -vf
        scale=1280:720 scaled.mp4"
  ...
  <nodes xsi:type="mediaflow:Resource"
      name="outputVideo"
      uri="file:///home/alfa/videos/output.mp4"
      external="true">
    <ports name="outputIn"/>
  </nodes>
...
\small
\vspace{6pt}
\begin{tikzpicture}[>=Latex, node distance=10mm, box/.style={draw,rounded corners,minimum width=1.6cm,minimum height=0.6cm,align=center,font=\tiny}]
  \node[box,fill=blue!15] (in) {inputVideo\\(Resource)};
  \node[box,fill=green!15,below=of in] (sc) {resize720\\(Scaler)};
  \node[box,fill=blue!15,below=of sc] (out) {outputVideo\\(Resource)};
  \draw[->,thick] (in) -- (sc);
  \draw[->,thick] (sc) -- (out);
\end{tikzpicture}
...
rule EntryResource2Initial {
    from r : Mediaflow!Resource
        (r.isEntry() and not r.isExit())
...
rule InternalResource2Action {
    from r : Mediaflow!Resource
        (not r.isEntry() and not r.isExit())
...
rule Edge2ControlFlow {
    ...
    to cf : UML!ControlFlow (
        name <- e.name,
        source <- e.sourceNode(),   -- resolves Port -> owner Node -> transformed UML node
        target <- e.targetNode()
    )
}
\end{lstlisting}
...
\vspace{2pt}
\small
\texttt{sourceNode()} dan \texttt{targetNode()} menyelesaikan indirection: ATL \textbf{otomatis} me-resolve referensi ke elemen target yang sesuai (trace-based resolution).
\end{frame}
...
\vspace{2pt}
\small
\textbf{Perbedaan arsitektural terbesar:} ATL membangun kontainer (\texttt{Model} + \texttt{Activity}) secara deklaratif di aturan \texttt{Graph2Model}. ETL melakukannya secara \textbf{imperatif di blok \texttt{post}} --- dieksekusi \textit{setelah} semua aturan selesai, menggunakan \texttt{.equivalent()} untuk mengambil elemen yang telah ditransformasi.
\end{frame}
...
\caption{Hasil transformasi \texttt{flow2}: topologi bercabang dipertahankan pada UML Activity Diagram.}
